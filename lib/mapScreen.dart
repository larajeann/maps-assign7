 ...
 void addMarker(LatLng position, String description) {
  markers.add(
    Marker(
      ...
      onTap: () => _promptDeleteDialog(position),
    ),
  );
  setState(() {});
}

void _promptDeleteDialog(LatLng position) {
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
        ...
        actions: <Widget>[
          ...
          TextButton(
            onPressed: () {
              deleteFavoritePlace(position.toString()); // Call delete method
              Navigator.of(context).pop(); // Close dialog
            },
            child: Text('Remove'),
          ),
...
void loadFavoritePlaces() async {
  ...
  querySnapshot.docs.forEach((doc) {
    ...
    // Ensure location is not null and contains latitude and longitude
    if (location != null && location is Map<String, dynamic> && location.containsKey('latitude') && location.containsKey('longitude')) {
      ...
      String description = doc['description'] ?? ''; // Ensure description is not null
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: GoogleMap(
        ...
        onTap: (position) {
          print(position);
          _promptDescriptionDialog(position);
        },
        markers: markers,
...
void _promptDescriptionDialog(LatLng position) {
  showDialog(
    context: context,
    builder: (BuildContext context) {
      return AlertDialog(
